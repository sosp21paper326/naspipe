...
def main():
    ...
    training_tensor_shapes = {"input0": list(input0.size()), "input1": list(input1.size()),
                              "target": list(target.size()), "ntokens": [1]}
    dtypes = {"input0": input0.dtype, "input1": input1.dtype,
              "target": target.dtype, "ntokens": torch.float32}
    ...
    def epoch_itr():
        return task.dataset('train').get_dummy_batch(args.max_tokens, max_positions)
    ...
    for t in range(total_updates):
        ...
        for i in range(accumulation - num_warmup_minibatches):
            ...
            if is_last_stage():
                ...
                if (t * accumulation + i + num_warmup_minibatches) % args.print_freq == 0:
                ...
                if i + num_warmup_minibatches == accumulation - 1 and (t * accumulation + i + num_warmup_minibatches) % args.print_freq < accumulation:
